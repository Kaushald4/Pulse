...
export async function generateCoverLetter(jobId: string): Promise<CoverLetterResult> {
  ...
  return withJobRun(label, () => "generated", async () => {
    ...
    let response;
    try {
      response = await callLlmStrict(COVER_LETTER_SYSTEM, resumeAndJobPrompt(resume.parsedText, job), false);
    } catch (error) {
...
export async function rewriteCoverLetter(
  ...
  return withJobRun(label, () => "rewritten", async () => {
    let response;
    try {
